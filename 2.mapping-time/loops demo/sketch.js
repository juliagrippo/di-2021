...
function draw(){
  ...
  _.times(numRows, r => {
    ...
    //  var grey = map(r, 0, numRows, 0, 255)
      //fill(grey)
      var grey = color('#DEDEDE')
      var red = color('#FF0000')

      var cellNumberPosition = map(cellNumber, 0, 60, 0, 100);
      var cellPosition = map(cellNumberPosition, 0, 100, 0, 1);
      var colorPosition = map(cellNumber,0,60,0,1);
      var fillColor = lerpColor(grey,red,colorPosition);
      fill(fillColor);
